...
class ProcessInputView1(APIView):
    """
    ...
    def post(self, request):
            try:
                est_new_quote_columns = [
                     "QuoteDate", "Quote_No", "ICompanyID", "ClientID", 
                    "Client_Name", "Product_Name", "Product_Code", "Carton_Type_ID",
                    "AUID", "ADateTime", "MUID", "MDateTime", "Remarks", "OrderStatus",
                    "IsActive", "FinalBy", "EnqNo", "DocNotion", "EstNotion", "FinalDate",
                    "RepID", "ImpExpStatus", "RevQuoteNo", "GrainStyle", "LocationID",
                    "CurrencyID", "Currency_Factctor", "Currency_CurrAmt", "ClientCategoryID",
                    "CalculatedRate", "QuoteRate", "FinalRate", "FPID"]
                est_qty_columns = ["QuoteID", "QtyReq"]
                est_board_columns = ["QuoteID", "BoardID", "Board_Type", "Board_GSM"]
                est_metpetp_columns = ["QuoteID", "MP_FT"]
                est_print_columns = ["QuoteID", "Pr_Complex", "Pr_FCol", "Pr_FB", "Pr_Add_Pl"]
                est_coating_columns = ["QuoteID", "FC_Type", "FC_Kind", "FC_Area", "FB"]
                est_lamination_columns = ["QuoteID", "FL_Film", "FL_Type", "FL_Strips", "FB"]
                est_foiling_columns = ["QuoteID", "FF_Film", "FF_L", "FF_B", "FB"]
                # data = request.data
                quote_id = insert_into_est_new_quote(request)
                if quote_id:
                    insert_into_est_qty(request,quote_id)
                    insert_into_table(request,"est_board",quote_id,est_board_columns)
                    insert_into_table(request,"est_metpetp",quote_id,est_metpetp_columns)
                    insert_into_table(request,"est_print",quote_id,est_print_columns)
                    insert_into_table(request,"est_coating",quote_id,est_coating_columns)
                    insert_into_table(request,"est_lamination",quote_id,est_lamination_columns)
                    insert_into_table(request,"est_foiling",quote_id,est_foiling_columns)
                ...
                #     self.insert_into_est_board(request,quote_id)
                #     self.insert_into_est_metpetp(request,quote_id)
                #     self.insert_into_est_print(request,quote_id)
                #     self.insert_into_est_coating(request,quote_id)
                #     self.insert_into_est_lamination(request,quote_id)
                #     self.insert_into_est_foiling(request,quote_id)
